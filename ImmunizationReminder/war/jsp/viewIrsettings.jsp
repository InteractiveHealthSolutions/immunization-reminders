...
<%@ include file="/WEB-INF/template/pagebody.jsp" %>
            <div class="function-tag">IR Settings</div>

<%
boolean perms=false;
try{
perms=UserSessionUtils.hasActiveUserPermission(SystemPermissions.VIEW_IRSETTINGS,request);
...
<form id="searchfrm" name="searchfrm" action="viewIrsettings.htm" method="post">
<div style="size:900px; overflow: scroll" >
    <table class="searchTable">
            <tr>
                <td>Enter setting name</td>
                <td><input type="text" id="settingName" name="settingName" value="${lastSearchSettingName}"/> </td>
           </tr>
			<tr>
                <td></td>
    ...
    <div style="color: red;font-size: small;font-style: italic">${updatesettingMsg}</div>
    <input type="hidden" id="action" name="action" >
    <input type="hidden" id="pagedir" name="pagedir" >
    
    <!-- for preserving search parameters for current pages -->
    <input type="hidden" id="lastSearchSettingName" name="lastSearchSettingName" value="${lastSearchSettingName}">
    ...
    <input type="hidden" id="totalRows" name="totalRows" value="${totalRows}">
            ...
            %>            	
            	...
            	<%@ include file="/portlets/viewIrsettinggrid.jsp" %>
</div>
</form>
<%
}else{
...
<c:out value="Sorry you must have permissions to view Settings. Contact your system administrator."></c:out>
</span>
<%
}
%>
