...
public class EntityValidation {
	...
	public boolean validateMrNumber( String childId, String mrNum, Date enrollmentDate ) {
		ERROR_MESSAGE = "";

		if (StringUtils.isEmptyOrWhitespaceOnly( mrNum )
				|| !DataValidation.validate( REG_EX.NUMERIC , mrNum.trim() )) {
			ERROR_MESSAGE = "Epi Number is empty or non numeric.";
			return false;
		}
		else {
			...
			if (!mrNum.trim().startsWith(Integer.toString( caldemin.get( Calendar.YEAR ) ) )
					|| !mrNum.trim().startsWith(Integer.toString( caldemax.get( Calendar.YEAR ) ) )) {
				ERROR_MESSAGE = "Epi Number should start with year of enrollment date ("
						+ caldemin.get( Calendar.YEAR ) + " OR " + caldemax.get(Calendar.YEAR) + ").";
				return false;
			}
			ServiceContext sc = Context.getServices();
			try {
				List<Child> p2 = sc.getChildService().findByEpiOrMrNumber(mrNum.trim() );
	...
	public boolean validateChildContactNumber( String cellNumber ,
