...
import org.ird.immunizationreminder.web.utils.ImporterFieldMap;
import org.ird.immunizationreminder.web.utils.UserSessionUtils;
import org.ird.immunizationreminder.web.validator.DataFieldValidator;
import org.ird.immunizationreminder.web.validator.EntityValidation;
...
public class UploadDataServlet extends HttpServlet {
	...
	protected void doPost( HttpServletRequest req , HttpServletResponse resp )
		...
		if (isMultipart) {
			...
			File fileTempRep = new File( tempDir.getPath() + IMRGlobals.UPLOAD_TEMP_DIR );
			...
			try {
				...
				while ( true ) {
					...
					if (!StringUtils.isEmptyOrWhitespaceOnly( line )) {
						String[] columnNames = line.split( IMRGlobals.CSV_DELIMITER_CHARACTER , -1 );
				...
				while ( ( line = input.readLine() ) != null ) {
					...
					if (!StringUtils.isEmptyOrWhitespaceOnly( line )) {
						...
						List<String> lineValues = new ArrayList<String>(
								Arrays.asList( line.split(IMRGlobals.CSV_DELIMITER_CHARACTER ,-1 ) ) 
								);
						...
						if (lineValues.size() != columnNamesList.size()) {
							...
							if (!isChildExisting) {
								...
								if (vaccination.getVaccinationDate() != null
									...
									Date actnextAssgnDate = IRUtils.calcNextVaccDuedate(
													vaccination.getVaccinationDate() ,
													vaccination.getVaccine() ,
													VACCINATION_GAP.USE_GAP_TO_NEXT_VACCINE_FIELD ,
													true );
									if (!StringUtils.isEmptyOrWhitespaceOnly(IRUtils.WARNING_MESSAGE)) {
										errMsg.add( IRUtils.WARNING_MESSAGE );
									}
								...
								nextVaccination.setVaccine(sc.getVaccinationService().getByName((vaccination.getVaccine().getName().toLowerCase().contains("bcg")?"Penta1/OPV":"Penta2/OPV")));
